  ...
  const updateContractStatus = (contractId, newStatus) => {
    ...
    if (updatedContract) {
      const newAuditLog = {
        ...
        actor: ROLES[userRole]?.name || 'Unknown', // Use user's name
        action: 'UPDATE_STATUS',
        target: contractId,
        details: `Contract status changed from ${updatedContract.status} to ${newStatus}.`
      };
      sampleAuditLogs.push(newAuditLog); // Directly modifying array for simplicity in demo
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: 'var(--bg-main)' }}>
      ...
      <main style={{ flexGrow: 1, padding: 'var(--spacing-lg) 0' }}>
        <div className="container">
            ...
            <section className="contract-list">
              ...
              <div className="grid-controls">
                ...
                <div className="flex gap-sm">
                  ...
                  {userPermissions.canCreateContract && (
                    <button className="btn btn-primary"><span className="icon icon-plus"></span> New Contract</button>
                  )}
                </div>
              </div>
              ...
              {filteredContracts.length === 0 ? (
                <div className="empty-state">
                  ...
                  {userPermissions.canCreateContract && (
                    <button className="btn btn-primary" onClick={() => {/* handle create new contract */}}>
          ...
          {view.screen === 'CONTRACT_EDIT' && userPermissions.canEditContract && (
            <section className="contract-edit">
              {(() => {
                ...
                // Initialize formData with contract data on component mount
                useEffect(() => {
          ...
          {!userPermissions[(`canView${view.screen.replace('_', '')}`)] && (
            <div className="empty-state">
